...
/**
 * Main class for the Man In The Middle SSL proxy. Delegates the real work to
 * HTTPSProxyEngine.
 */
...
public class MITMProxyServer
{
   ...
   private MITMProxyServer(String[] args)
   {
      ...
      try
      {
         for(int i = 0; i < args.length; i++)
         {
            ...
            {
               // Here is where the password is saved
               System.setProperty(JSSEConstants.KEYSTORE_PASSWORD_PROPERTY,
                                  args[++i]);
            }
      ...
      if(timeout < 0)
      {
         throw this.printUsage("Timeout must be non-negative");
      }
      ...
      try
      {
         this.m_engine = new HTTPSProxyEngine(new MITMPlainSocketFactory(),
                                              ...
                                              localPort,
                                              timeout);
         this.m_adminServer = new MITMAdminServer(localHost,
                                                  adminPort,
                                                  this.m_engine);
   ...
   public void run()
   {
      (new Thread(this.m_adminServer)).start();
      this.m_engine.run();
